...
def avg_pool(input_layer, **kwargs):
    ...
    norm = ExpressionLayer(input_layer, lambda X: T.ones_like(X))
    norm = Pool2DLayer(norm, mode='average_inc_pad', **kwargs)
    l = Pool2DLayer(input_layer, mode='average_inc_pad', **kwargs)
    l = ElemwiseMergeLayer([l, norm], T.true_div)
    return l
...
def inceptionA(input_layer, nfilt):
    ...
    l4 = avg_pool(input_layer, pool_size=3, stride=1, pad=1)
...
def inceptionE(input_layer, nfilt, pool_type):
    l1 = bn_conv(input_layer, num_filters=nfilt[0][0], filter_size=1)
    ...
    if pool_type == 'avg':
        ...
        l4 = Pool2DLayer(input_layer, pool_size=3, stride=1, pad=1, mode='max')
    l4 = bn_conv(l4, num_filters=nfilt[3][0], filter_size=1)
